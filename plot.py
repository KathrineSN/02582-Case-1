...
#%% Mean Loadfactor for each date
df_init['dates'] = df_init.ScheduleTime.apply(lambda text: str(text).split(' ')[0])

df_date_loadfactor = df_init[['dates','LoadFactor']].groupby(['dates']).mean()
plt.plot(df_date_loadfactor)
